...
const UpdateProfile = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.put('http://localhost:5000/api/students/profile', formData, {
        headers: {
        'y-auth-token': localStorage.getItem('studenttoken'),
      }},);
      setFormData(response.data);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-blue-400 to-green-400 flex items-center justify-center rounded-2xl">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        <h2 className="text-2xl font-bold mb-6 text-center text-black">Update Profile</h2>
